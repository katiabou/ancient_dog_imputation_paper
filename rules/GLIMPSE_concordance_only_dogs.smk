...
rule prepare_ref_panel_only_dogs:
    ...
    output:
        ref_concordance_sample_excl = temp('{path}/output/GLIMPSE_concordance_only_dogs/reference_panel_only_dogs/{chrom}_ref_panel.phased.vcf.gz')
    log:
...
rule fill_tags_ref_sample_excl_only_dogs:
    ...
    output:
        ref_concordance_sample_excl_filltags = temp('{path}/output/GLIMPSE_concordance_only_dogs/reference_panel_only_dogs/{chrom}_ref_panel_filltags.phased.vcf.gz')
    log:
...
rule prepare_merged_chr_list_ref_only_dogs:
    ...
    """
    input:
        ref_concordance_sample_excl_filltags_filter = expand('{path}/output/GLIMPSE_concordance_only_dogs/reference_panel_only_dogs/{chrom}_ref_panel_filltags_filter.phased.bcf', chrom=CHROM, allow_missing=True),
    output:
...
rule prepare_merged_chr_list_concordance_only_dogs:
    ...
    """
    input:
        info_imputed_info = expand('{path}/output/GLIMPSE_concordance_only_dogs/GLIMPSE_ligated_INFO_filtered/merged_ligated.{sample}_{chrom}_{coverage_val}x-INFO_{info_cutoff}.bcf', chrom=CHROM, allow_missing=True),
    output:
...
rule prepare_concordance_output_filt_allchrom_only_dogs:
    ...
    input:
        concordance_output_discordance_filtered = '{path}/output/GLIMPSE_concordance_only_dogs/concordance_INFO_filtered/concordance_{sample}_allchrom_{coverage_val}x-INFO_{info_cutoff}_filtered.error.spl.txt.gz'
    output:
        concordance_output_discordance_filtered_temp = temp('{path}/output/GLIMPSE_concordance_only_dogs/concordance_INFO_filtered/temp_concordance_{sample}_allchrom_{coverage_val}x-INFO_{info_cutoff}_filtered.txt'),
...
rule merge_concordance_output_filt_allchrom_only_dogs:
    ...
    """
    input:
        concordance_output_discordance_filtered_prep = expand('{path}/output/GLIMPSE_concordance_only_dogs/concordance_INFO_filtered/concordance_{sample}_allchrom_{coverage_val}x-INFO_{info_cutoff}_filtered.txt', coverage_val=COVERAGE_VAL, info_cutoff=INFO_CUTOFF, allow_missing=True)
    output:
...
rule plot_discordance_filt_allchrom_only_dogs:
    ...
    """
    input:
        concordance_output_discordance_filtered_per_sample = expand('{path}/output/GLIMPSE_concordance_only_dogs/concordance_INFO_filtered/concordance_{sample}_allchrom_filtered.txt', sample=SAMPLE, allow_missing=True),
        concordance_metadata = config['bam_targets']
    params:
        path_script = '{path}/scripts',
        discordance_phased=lambda wildcards, input: ','.join(input.concordance_output_discordance_filtered_per_sample),
...
rule plot_concordance_filt_allsites_only_dogs_0_5x_1x:
    input:
        ...
        concordance_output_info_score_filtered_only_dogs_mod = expand('{path}/output/GLIMPSE_concordance_only_dogs/concordance_INFO_filtered/concordance_{sample}_allchrom_{coverage_val}x-INFO_{info_cutoff}_filtered.rsquare-mod.grp.txt.gz', sample=SAMPLE, info_cutoff=INFO_CUTOFF, coverage_val=['0.5','1'], allow_missing=True),
        concordance_metadata = config['bam_targets']
    output:
    ...
    params:
        files_all=lambda wildcards, input: ','.join(input.concordance_output_info_score_filtered_mod),
        files_dogs=lambda wildcards, input: ','.join(input.concordance_output_info_score_filtered_only_dogs_mod)
    shell:
        '''
