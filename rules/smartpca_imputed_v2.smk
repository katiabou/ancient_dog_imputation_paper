...
rule canid_subset_prepare_merged_chr_list:
    ...
    """
    input:
        modern_imputed_subset = expand('{path}/output/GLIMPSE_imputation/PCA/merged_phased_modern_vcf/merged_modern_phased_annotated.{chrom}_MAF_{maf_cutoff}_recalibrated_INFO_{info}_{canid_subset}.vcf.gz', chrom=CHROM, allow_missing=True)
    output:
...
rule make_plink:
    ...
    input:
        merged_vcf = '{path}/output/GLIMPSE_imputation/PCA/merged_phased_modern_vcf/merged_modern_phased_annotated.allchrom_MAF_{maf_cutoff}_recalibrated_INFO_{info}_{canid_subset}.vcf.gz'
    output:
        expand('{path}/output/GLIMPSE_imputation/PCA/merged_phased_modern_vcf/merged_modern_phased_annotated.allchrom_MAF_{maf_cutoff}_recalibrated_INFO_{info}_{canid_subset}.{doc}', doc=DOCS, allow_missing=True)
    params:
...
rule fix_chr_column:
    ...
    output:
        temp_mod_bim = temp('{path}/output/GLIMPSE_imputation/PCA/merged_phased_modern_vcf/merged_modern_phased_annotated.allchrom_MAF_{maf_cutoff}_recalibrated_INFO_{info}_{canid_subset}-mod-temp.bim'),
