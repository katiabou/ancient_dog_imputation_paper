...
ggplot(wolves, aes(x = Age_Mean_KBP, y = froh)) +
    ...
    scale_x_continuous(breaks = seq(round(min(wolves$Age_Mean_KBP + 1)), 0, 10)) +
    geom_label_repel(
        data = wolves %>% filter(type != "modern"),
...
ggplot(wolves_pleist, aes(x = Age_Mean_KBP, y = froh)) +
    ...
    scale_x_continuous(breaks = seq(round(min(wolves_pleist$Age_Mean_KBP + 1)), 0, 10)) +
    geom_label_repel(
        data = wolves_pleist %>% filter(type != "modern" & froh > 0.0002),
        aes(x = Age_Mean_KBP, y = froh, label = Sample), size = 2, box.padding = 1, max.overlaps = Inf
    ) +
    labs(x = "Time (kya)", y = expression(paste(italic("F")[ROH]))) +
    theme_bw() +
...
# filter out wolves and unwanted outgroup populations:
wolves <- ab_long %>% filter(Dog_PCA == "Wolves" & Wolf_Dog_PCA != "Outgroup" & Wolf_Dog_PCA != "Wolves")
...
# filter out wolves and unwanted outgroup populations:
wolves <- ab_short %>% filter(Dog_PCA == "Wolves" & Wolf_Dog_PCA != "Outgroup" & Wolf_Dog_PCA != "Wolves")
...
# ##### WOLVES, would not trust this probably ####
# final_map_imputed_wolves <- final_map %>% filter(Species=='lupus' & type=='imputed')
...
# sites_2 <- final_map_imputed_wolves
...
#   coord_fixed(ratio = 1, xlim = c(-160,190), ylim = c(20, 80)) +
...
#   geom_label_repel(data = sites_2 %>% filter(froh > 0.01),
