...
newData <- melt(list(INFO_0.8 = INFO_0.8[c(3,5)], INFO_0.9 = INFO_0.9[c(3,5)],
                     INFO_0.95 = INFO_0.95[c(3,5)], ALL_INFO = ALL_INFO[c(3,5)]), id.vars = "x")
...
Avgs <- test %>% group_by(Group) %>% summarize(Avg = mean(x))
breaks <- log(Avgs[,2])
