...
b <- str_split(files_dogs, pattern=',')
...
colnames(dogs) <- c('V1','V2','V3','V4','V5','V6','V7','V8')
...
#add all sites or dog info to dataframe
all$type <- 'All canid reference panel'
dogs$type <- 'Dog reference panel'

#merge two datasets
df_both <- rbind(all, dogs)
...
df_both$type_b <- factor(df_both$type, levels=c('All canid reference panel','Dog reference panel'))
df_both$V8_b <- factor(df_both$V8, levels=c('NGDG','SOTN01_merged','PortauChoix','TRF.02.53','TRF.05.05',
                                            'FAMICHN00012','FAMINGR00004','CGG32','CGG33','WolfHead'))

#png(args[2], width=25, height=16, units='in', res=250, pointsize=4)

p1 <- df_both %>% filter(species=='Dogs' & V6==0.5) %>%
...
p2 <- df_both %>% filter(species=='Pleistocene_wolf' & V6==1) %>%
  ggplot(aes(log(V3), V4, colour=as.character(V7)))+
  #geom_rect(xmin = breaks$Avg[5], xmax = max(breaks$Avg), ymin = -0.5, ymax = 1.5,
  #          fill = 'gray80', colour = 'white', alpha = 0.01) +
  geom_line(linetype = "solid", size=1.5)+
