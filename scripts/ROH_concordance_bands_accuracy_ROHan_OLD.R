...
rohan2 <- rohan %>% filter(hEst=='mid'& FID==name) %>%
  select(FID, IID, CHR, POS1, POS2, KB, NSNP, cov)
...
all_seg_stats_roh <- rbind(get_seg_stats_a_roh,
                           get_seg_stats_b_roh,
                           get_seg_stats_c_roh,
                           get_seg_stats_d_roh,
                           get_seg_stats_e_roh,
...
m1 <- ggplot(all_length_stats_all, aes(x=cov, y=mcc_norm, linetype=Data))+
  ...
  geom_point(size=4, colour="lightseagreen")+
...
met1 <- all_stats_final %>% filter(metric=='specificity') %>% 
...
met2 <- all_stats_final %>% filter(metric=='sensitivity') %>% 
  ggplot(aes(x=cov, y=values, linetype=Data))+
...
met3 <- all_stats_final %>% filter(metric=='FDR') %>% 
  ggplot(aes(x=cov, y=values, linetype=Data))+
...
met_all <- ggarrange(m1,met1,met2, met3,
                  #labels=c("a","b","c","d"),
                  ncol = 4, nrow = 1, 
                  font.label=list(size=20),
                  common.legend=TRUE,
...
png(args[[18]], width=22, height=5, units='in', res=200, pointsize=4)
#png('~/Downloads/test.png', width=22, height=5, units='in', res=200, pointsize=4)
annotate_figure(met_all, top = text_grob(name_title_final, face = "bold", size = 18))
dev.off()
