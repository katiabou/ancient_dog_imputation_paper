...
genome_kmer_bad_wolf_allchrom <- read.delim(snakemake@input[[2]], header=FALSE)
...
kmer_depth_mean <- genome_kmer_bad_wolf_allchrom %>%
  group_by(V1,V2,V3) %>%
...
p1 <- kmer_depth_mean_no_grey %>%
  ...
  geom_density(fill="coral3", color="#e9ecef", alpha=0.8) +
  theme_bw()+
...
kmer_depth_mean_no_grey_filt <- kmer_depth_mean_no_grey %>% filter(weighted_mean_frequency<0.19) #cutoff for modern wolves



####################################### CNV frequencry against ROH prevelance  ######################################

# merge whole genome kmer with ROH prevelance from modern wolves
...
### number of ROH deserts left in modern wolves:
...
kmer_depth_mean_no_grey_filt_remove <- kmer_depth_mean_no_grey %>% 
  filter(weighted_mean_frequency>=0.19) #cutoff for modern wolves
