...
df %>% 
  ...
  geom_rect(aes(xmin = POS1, xmax = POS2, ymin = yax - 0.5, ymax = yax + 0.9, 
                fill = Meta.Population), size = 0, alpha = 1) + 
...
wolf_list <- c('Wolf27','Wolf20','Wolf24','Wolf39','Wolf05',
...
yax_1 <- df_1 %>% 
  arrange(Meta.Population = factor(Meta.Population, levels=c("Eastern_Eurasian_Wolves", 
                                                       "Western_Eurasian_Wolves", 
                                                       "North_American_Wolves", 
                                                       "Pleistocene_Wolves")),Age_Mean_BP) 
...
df %>% 
  ...
  geom_rect(aes(xmin = POS1, xmax = POS2, ymin = yax - 0.5, ymax = yax + 0.9, 
                fill = Meta.Population), size = 0, alpha = 1) + 
  scale_fill_manual(values = cols, breaks = c("Eastern_Eurasian_Wolves", "Western_Eurasian_Wolves", "North_American_Wolves", "Pleistocene_Wolves"),
